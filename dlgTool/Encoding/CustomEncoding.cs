...
namespace dlgTool.CustomEncoding
{
    ...
    public class AAEncoding
    {
            ...
            using (var br = new BinaryReaderY(new MemoryStream(input)))
            {
                ...
                for (int i = 0; i < vals.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        List<(string, int, bool)> textList; 
                        if (i < vals.Count - 1)
                            textList = GetTextList(br.BaseStream, vals[i + 1]);
                        else
                            textList = GetTextList(br.BaseStream); 
        ...
        private List<(string, int, bool)> GetTextList(Stream input, int next)
        {
            var result = new List<(string, int, bool)>();

            using (var br = new BinaryReaderY(input, true))
            {
                while (br.BaseStream.Position < br.BaseStream.Length && br.BaseStream.Position < next)
                {
                    var code = br.ReadInt16();
                    if (code < 0x90)
                    {
